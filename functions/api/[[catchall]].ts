/**
 * CF Pages catch-all function for /api/* routes.
 *
 * Mirrors the Vercel entry point (api/[domain]/v1/[rpc].ts) but runs
 * as a Cloudflare Pages Function. The _middleware.ts has already
 * populated globalThis.process.env from CF bindings before this runs.
 *
 * Handles:
 *   - All sebuf RPC routes (POST /api/{domain}/v1/{rpc})
 *   - Version endpoint (GET /api/version)
 */

// --- Sebuf router + service imports (same as Vercel entry point) ---
...
// --- Build the router (runs once at module load) ---
...
function handleVersion(): Response {
  return new Response(JSON.stringify({
    ...
    variant: process.env.VITE_VARIANT || 'finance',
...
export async function onRequest(context: CFPagesContext): Promise<Response> {
  ...
  try {
    ...
    console.error('[cf-gateway] Unhandled handler error:', err);
